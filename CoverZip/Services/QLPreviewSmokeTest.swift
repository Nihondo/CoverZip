//
//  QLPreviewSmokeTest.swift
//  CoverZip
//
//  Quick Look 埋め込みの最小経路のみで動作確認するためのスモークテスト。
//  - 余計なイベントモニタや合成クリックは一切行わない
//  - ウィンドウクローズは OS の標準挙動に任せる
//
...
/// Quick Look 埋め込みの単機能テストを提供するユーティリティ
enum QLPreviewSmokeTest {
    ...
    /// 指定 URL を QLPreviewView 単体で表示するテストウィンドウを開く
    static func openQuickLookWindow(url: URL, enableKeyMonitors: Bool = false) {
        DispatchQueue.main.async {
            ...
            window.title = "QL Test – " + url.lastPathComponent
            window.center()
    ...
    private final class KeyForwardingView: NSView {
        ...
        override func keyDown(with event: NSEvent) {
            NSLog("[QLSmokeTest] keyDown code=%d flags=%lu", Int(event.keyCode), event.modifierFlags.rawValue)
            switch Int(event.keyCode) {
