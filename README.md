...
- **段階的プレビュー（現在は全ホストで無効化中）**: 巨大 JPEG を二段階で描画する機能は Finder カラムで不具合が出るため一時停止。再開する場合は Finder 判定ロジックと表示サイズ更新の挙動を検証してから有効化してください。
...
- ImageManager の先読みロジックはアプリと Quick Look で共通ですが、現在は段階的JPEGデコードおよびページ移動時のプリロードキャンセルを全ホストで停止しています（表示乱れ対策）。
