...
- **段階的プレビュー**: 巨大 JPEG は低解像度プレビューを即時描画し、その後バックグラウンドで高解像度へ差し替え（`imageManagerDidUpdateImage` 通知で UI を更新）
...
- ImageManager の先読み/段階的デコードはアプリ内ビューアでも共有され、見開きモードでは常に次の見開き分まで読み込みます。
...
- 先読みや段階的デコードの挙動は `[ImageManager] Preload plan ...` / `[ImageManager] Preload completed ...` ログで確認できます。
