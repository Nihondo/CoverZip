...
- **遅延ロード方式**: メタデータを先行取得し、画像データは必要時にロード
  ...
  - 隣接ページの自動プリロード
- **スケーリングデコード**: 表示サイズに合わせて ImageIO のダウンサンプリング / サブサンプリングを実施し、4K 超の JPEG でも初期描画負荷を抑制
- **段階的プレビュー（現在 Finder カラムでは無効化中）**: 巨大 JPEG は低解像度プレビューを即時描画し、その後バックグラウンドで高解像度へ差し替え（Quick Look 独立ウィンドウのみ。Finder カラムでは安全のためフル解像のみを使用）
- **モード連動プリロード**: 単ページ時は ±1→±2 ページ、見開き時は ±2→±4 ページを段階的に先読みし、連続ページ送りやスライドショーでも待ち時間を最小化
...
- ImageManager の先読みロジックはアプリと Quick Look で共通ですが、Finder カラム検出時は段階的JPEGデコードを自動的にオフにし、フル解像度のみを使用します。
...
- Quick Look Extension（preview/thumbnail）の `NSLog` は `quicklookd` プロセスに出力されます。Console.app で `process == quicklookd` をフィルタするか `log stream --predicate 'process == "quicklookd"'` を使用してください。
- 先読みや段階的デコードの挙動は `[ImageManager] Preload plan ...` / `[ImageManager] Preload completed ...` ログで確認できます。Finder カラム判定ログ `[HostDetect reason=...]` も合わせて参照すると、段階的デコードが無効になっているかどうかを確認できます。
- Finder カラムの判定はビュー/ウィンドウの実サイズ（幅 < 620px など）と Quick Look ホストクラス名のヒューリスティックを併用しています。誤判定が疑われる場合は `NSLog` のサイズ推定値を添えて報告してください。
