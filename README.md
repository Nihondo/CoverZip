...
- **ページ移動**は、画面の左右をクリックするか、マウスホイールの上下スクロールで行います。矢印キーは QuickLook ビューアでは使えません（Finder にキーが奪われるため。後述のアプリ内蔵ビューアでは使えます）。
...
- ファイル名に「コミック」が含まれていれば CoverZip のアプリ内蔵ビューアで開く
