...
画像を先にZIPにまとめる必要はありません。Finderで画像が入ったフォルダを選んでスペースキーを押すと、上記と同じビューア・同じ操作方法・同じ読書履歴機能でそのフォルダを閲覧できます。サブフォルダの中身も自動的に含まれ、ZIP内の画像と同じ自然順（`2.jpg` が `10.jpg` より先）でソートされます。画像が1枚も入っていないフォルダの場合は、Finder標準のフォルダプレビューが表示されます。
...
CoverZip が対応しているのは、標準的な ZIP ファイル（.zip）と、画像が入った通常のフォルダです（詳しくは[フォルダの中の画像をそのままプレビューする](#フォルダの中の画像をそのままプレビューする)を参照）。JPEG、PNG、GIF、BMP、TIFF、WebP などの一般的な画像形式を認識します。

パスワードで保護された ZIP ファイルや、Zip64 形式、DEFLATE 以外の圧縮方式には対応していません。また、フォルダのFinderサムネイル生成には対応しておらず（サムネイルにカバー画像が表示されるのはZIPファイルのみ）、フォルダ対応はスペースキーでのQuickLookと内蔵ビューアに限られます。
