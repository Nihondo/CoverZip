...
フォルダのQuickLookプレビューは、ZIP/RARアーカイブとは別のQuickLook機能拡張が担当しています。他のアプリのフォルダプレビューを使いたい場合（またはフォルダのQuickLookプレビュー自体を使いたくない場合）は、**システム設定 > 一般 > ログイン項目と機能拡張 > クイックルック** で「coverZipFolderViewer」をオフにしてください。これはスペースキーによるフォルダのプレビューのみに影響し、ZIP/RARのプレビューや内蔵ビューアはどちらの設定でも引き続き動作します。
