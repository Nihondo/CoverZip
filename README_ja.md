...
### キー操作
内蔵ビューアでは、クリックやスクロールに加えて次のキー操作が使えます。任意機能の Finder QuickLook 用キー入力ヘルパーを有効にすると、Space 系ショートカットを除く同じナビゲーションキーを Finder QuickLook でも使えます。矢印キーの前後は、読み方向の設定（右綴じ / 左綴じ）に応じて自動的に切り替わります。
